...
class MarketSimulationStateUpdate(SimulationStateUpdate):
    ...
    @classmethod
    def from_simulator(self, timestamp, message : SimulatorStateUpdate):
        """
    ...
    def compress(self):
        ...
        try:
            if not self.compressed:
                ...
                if self.books != {}:
                    compressed.books = base64.b64encode(zlib.compress(json.dumps({bookId : book.model_dump(mode='json') for bookId, book in self.books.items()}).encode('utf-8'))).decode("ascii")
                    compressed.accounts = base64.b64encode(zlib.compress(json.dumps({accountId : {bookId : account.model_dump(mode='json') for bookId, account in accounts.items()} for accountId, accounts in self.accounts.items()}).encode('utf-8'))).decode("ascii")
                    compressed.notices = base64.b64encode(zlib.compress(json.dumps({agentId : [notice.model_dump(mode='json') for notice in notices] for agentId, notices in self.notices.items()}).encode('utf-8'))).decode("ascii")
                    compressed.config = base64.b64encode(zlib.compress(json.dumps(compressed.config.model_dump(mode='json')).encode('utf-8'))).decode("ascii")
                if compressed.response:
                    compressed.response = base64.b64encode(zlib.compress(json.dumps(compressed.response.model_dump(mode='json')).encode('utf-8'))).decode("ascii")
    ...
    def decompress(self):
        ...
        try:
            if self.compressed:
                if self.books != {}:
                    self.books = json.loads(zlib.decompress(base64.b64decode(self.books)))
                    self.accounts = json.loads(zlib.decompress(base64.b64decode(self.accounts)))
                    self.notices = json.loads(zlib.decompress(base64.b64decode(self.notices)))
                    self.config = json.loads(zlib.decompress(base64.b64decode(self.config)))
                if self.response:
                    self.response = json.loads(zlib.decompress(base64.b64decode(self.response)))
