        ...
        try:
            ...
            def connect() -> None:
                ...
                while True:
                    attempts += 1
                    self.seed_exchange='coinbase'
                    self.seed_client = connect_coinbase([self.config.simulation.seeding.fundamental.symbol.coinbase, self.config.simulation.seeding.external.symbol.coinbase], on_coinbase_trade)
                    if not self.seed_client:
                        ...
                        self.seed_client = connect_binance([self.config.simulation.seeding.fundamental.symbol.binance,self.config.simulation.seeding.external.symbol.binance], on_binance_trade)
                        if not self.seed_client:
